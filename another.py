...
def on_click(event):
    ...
    try:
        if text == "=":
            display_text.set(eval(display_text.get().replace("^", "**")))
        elif text == "C":
            display_text.set("")
        elif text == "√":
...
tk.Entry(root, textvariable=display_text, font=("Arial", 24), justify="right", relief=tk.FLAT, bd=10, bg="#ECF0F1", fg="#2C3E50").pack(fill=tk.BOTH, ipadx=8, padx=10, pady=10)
...
def create_button(parent, text):
    ...
    btn.pack(side=tk.LEFT, fill=tk.BOTH, padx=5, pady=5, expand=True)
